...
function FormList() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://form2-7uno.onrender.com/getallform');
  ...
  const generateCSV = forms => {
    ...
    const rows = forms.map((form, index) => {
      return `${index + 1},${form.WhatAreYouInterestedIn},${form.name},${form.email},${form.phone},${form.WhenDoYouNeedToMove},${form.HowManyPeople},${form.HowLongFor},${form.CompanyName},${form.CompanySize}\n`;
    });
